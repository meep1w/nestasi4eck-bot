...
from app.routers.admin import postbacks as postbacks_router  # <— новый
...
router.include_router(postbacks_router.router)  # <— подключили
...
async def set_last_bot_message_id(tg_id: int, message_id: Optional[int]):
    async with async_session() as session:
        ...
        if not user:
            user = User(id=tg_id)  # на всякий случай, админ мог не запускать /start
            session.add(user)
...
async def render_one_window(ctx, text: str, kb: InlineKeyboardMarkup):
    """
    Отрисовать одно окно: удалить предыдущее сообщение бота (если было) и отправить новое.
...
async def cmd_admin(m: Message):
    ...
    await render_one_window(m, text, kb_admin_root())


# === callbacks (root only: back) ===
...
async def cb_admin_back(call: CallbackQuery):
    ...
    await render_one_window(call, text, kb_admin_root())
